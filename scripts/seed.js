...
const uri = 'mongodb+srv://achiz:A124563a@karaoke.xhh0ave.mongodb.net/?retryWrites=true&w=majority&appName=Karaoke';
if (!uri) {
...
const roomSubSchema = new mongoose.Schema(
  {
    ...
    type:   { type: String, required: true, enum: ["STANDARD","PREMIUM","DELUXE","VIP"] },
    price:  { type: Number, required: true, min: 0 },
  },
  { _id: false }
);
...
const bookingSchema = new mongoose.Schema(
  {
    ...
    customerEmail: { type: String, required: true, lowercase: true, trim: true, match: [/^\S+@\S+\.\S+$/, "Invalid email"] },
    customerPhone: { type: String, required: true, trim: true },
    date:          { type: Date, required: true, index: true },
    timeSlot:      { type: String, required: true, trim: true, match: [/^([01]\d|2[0-3]):[0-5]\d-([01]\d|2[0-3]):[0-5]\d$/, "Invalid timeSlot (HH:MM-HH:MM)"] },
    numberOfPeople:{ type: Number, required: true, min: 1 },
    status:        { type: String, required: true, enum: ["PENDING","CONFIRMED","PAID","COMPLETED","CANCELLED","REFUNDED"], default: "PENDING", index: true },
...
async function main() {
  ...
  const users = await User.insertMany([
    ...
    { name: "John Doe", email: "john@example.com", password: hash, role: "user" },
    { name: "Jane Smith", email: "jane@example.com", password: hash, role: "user" },
  ]);
  console.log(`👤 Users: ${users.length} (admin@example.com / password123)`);

  // Rooms
  const rooms = await Room.insertMany([
    { name: "VIP Room Gold",   number: "V201", type: "VIP",      capacity: 12, price: 2500, status: "AVAILABLE" },
    { name: "Premium Suite A", number: "A101", type: "PREMIUM",  capacity: 8,  price: 1600, status: "AVAILABLE" },
    { name: "Premium Suite B", number: "A102", type: "PREMIUM",  capacity: 8,  price: 1700, status: "AVAILABLE" },
    { name: "Deluxe Suite",    number: "D301", type: "DELUXE",   capacity: 10, price: 1800, status: "AVAILABLE" },
    { name: "Standard Room 1", number: "S101", type: "STANDARD", capacity: 6,  price: 800,  status: "AVAILABLE" },
    { name: "Standard Room 2", number: "S102", type: "STANDARD", capacity: 6,  price: 800,  status: "MAINTENANCE" },
  ]);
  ...
  const promotions = await Promotion.insertMany([
    { code: "NEWUSER10",  name: "New User 10%",     discountType: "PERCENT", discountValue: 10,  startDate: addMonths(now, -2), endDate: addMonths(now, 2), isActive: true },
    { code: "WEEKEND100", name: "Weekend ฿100 Off", discountType: "FIXED",   discountValue: 100, startDate: addMonths(now, -1), endDate: addMonths(now, 1), isActive: true },
    { code: "VIP500",     name: "VIP ฿500 Off",     discountType: "FIXED",   discountValue: 500, startDate: addMonths(now, -3), endDate: addMonths(now, 3), isActive: true },
  ]);
  ...
  for (let i = 1; i <= 30; i++) {
    ...
    bookingsData.push({
      ...
      numberOfPeople: Math.min(r.capacity, 2 + Math.floor(Math.random() * r.capacity)),

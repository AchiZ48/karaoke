...
export async function PATCH(req, context) {
  try {
    ...
    const isOwner = session.user?.email && booking.customerEmail === session.user.email;
    const isAdmin = session.user?.role === 'admin';

    if (action === 'cancel') {
      if (!isOwner && !isAdmin) return NextResponse.json({ message: "Forbidden" }, { status: 403 });
      if (["COMPLETED","CANCELLED","REFUNDED"].includes(booking.status)) {
        return NextResponse.json({ message: "Cannot cancel this booking" }, { status: 400 });
      }
    ...
    if (isAdmin && (newDate || newTimeSlot)) {
      if (!newDate || !newTimeSlot) return NextResponse.json({ message: 'date and timeSlot are required' }, { status: 400 });
      // Validate format similar to bookings POST
      const isValidTimeSlot = /^([01]\d|2[0-3]):[0-5]\d-([01]\d|2[0-3]):[0-5]\d$/.test(newTimeSlot);
      if (!isValidTimeSlot) return NextResponse.json({ message: 'Invalid timeSlot' }, { status: 400 });
      const reqDate = new Date(newDate); reqDate.setHours(0,0,0,0);
      const today = new Date(); today.setHours(0,0,0,0);
      if (reqDate < today) return NextResponse.json({ message: 'Date cannot be in the past' }, { status: 400 });
      ...
      if (isToday) {
        ...
        const slotEnd = new Date(reqDate); slotEnd.setHours(eh, em, 0, 0);
        if (slotEnd.getTime() <= now.getTime()) return NextResponse.json({ message: 'Selected time slot already passed' }, { status: 400 });
      }
      ...
      if (exists) return NextResponse.json({ message: 'Selected time slot is unavailable' }, { status: 409 });

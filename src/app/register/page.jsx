...
function register() {
    ...
    const handleSubmit = async (e) => {
        ...
        if(password !== confirmpassword){
            setError("Password do not match!"); showToast('Passwords do not match','error');
            return;
        }
        if(!name || !email || !password || !confirmpassword || !phone){
            setError("Please complete all inputs!"); showToast('Please complete all inputs','error');
            return;
        }
        if (!isStrongPassword(password)) {
            setError(`Weak password. ${PASSWORD_REQUIREMENTS}.`); showToast('Weak password','error');
            return;
        }
        try {
            ...
            if(user){
                setError("User already exists!"); showToast('User already exists','error');
                return;
            }
            ...
            if(res.ok){
                setError("");
                setSuccess("User registration successfully! Logging you in..."); showToast('Registration successful','success');
                // Auto sign-in after register
                await signIn('credentials', { email, password, callbackUrl: '/' });
            }else{
                console.log("User registration failed."); showToast('Registration failed','error');
            }
        } catch (error) {
            console.log("Error while registration: ", error); showToast('Registration error','error');
        }
    }
  return (
    <div>
      <div className='container mx-auto py-5'>
        ...
        <form onSubmit={handleSubmit}>
            ...
            <p className='text-xs opacity-70 -mt-1 mb-2'>{PASSWORD_REQUIREMENTS}</p>
            ...
            <input onChange={(e) => setPhone(e.target.value)} className='block bg-gray-300 p-2 my-2 rounded-md' type="tel" placeholder='Phone number'/>
